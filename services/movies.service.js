...
async function createMovieService({ Title, Year, Runtime, Genre, Director, Actors, Plot, Country, Poster, imdbRating, Opinions }) {
  try {
  const newMovie = new Movie({
    ...
    return { error: true, message: "Error al crear el pelicula", details: error.message };
  }
}
...
const editMovieService = async ({title,datosEditarMovie})=>{
    return await Movie.findOneAndUpdate(  { Title: title }, datosEditarMovie, { new: true }); //Método de Mongoose que busca un documento por su ID y lo actualiza .findByIdAndUpdate()
}
